...
// ---------------------------------------------------------------------
#ifndef CAL_MAX_SAMPLES
  #define CAL_MAX_SAMPLES  20     // maksimal 20 sampel berguna (~40 detik) - gaya instrumen bangku
#endif
...
static bool collectStableReading(double &resultPh, double &resultMv, bool &wasStable, const char *label,
                                  const char *titikDb, const char *aksiDb) {
  unsigned long startTime = millis();
  int totalSamples = 0, mvCount = 0, sampelBerguna = 0;
  double mvSum = 0;
  ...
  Serial.printf("Maksimal %d sampel (~%lu detik) setelah warm-up.\n",
                CAL_MAX_SAMPLES, (unsigned long)CAL_MAX_SAMPLES * CAL_SAMPLE_MS / 1000UL);
  ...
  while (true) {
    ...
    if (ok && !inWarmup) {
      ...
      mvSum += mv; mvCount++;
      sampelBerguna++;
      kirimSampleKalibrasi(titikDb, aksiDb, sampelBerguna, ph, tc, mv);
    }
    ...
    bool cukupSampel = (sampelBerguna >= CAL_MAX_SAMPLES);

    if (!inWarmup && ((s.valid && s.stable) || cukupSampel)) {
      const char *finalMsg;
      Serial.println(F("\n--------------------------------"));
      if (!s.valid) {
        ...
        Serial.println(F("STATUS: BELUM STABIL sampai batas sampel."));
...
static void calibratePoint(int idx, uint16_t step, const char *label, const char *prefsKey) {
  ...
  const char *titikDb = (idx == 0) ? "ph4" : (idx == 1) ? "ph7" : "ph10";
  double ph = 0, mv = NAN; bool wasStable = false;
  if (!collectStableReading(ph, mv, wasStable, label, titikDb, "kalibrasi")) return;
  if (!wasStable) {
  ...
  if (calibrated) {
    ...
    Serial.println(F("Kedua titik selesai. Lanjut verifikasi: 'A' (4.01), '7' (6.86), 'C' (9.18)."));
  }
}
...
static void verifyPoint(int idx, const char *label, const char *prefsKey) {
  ...
  const char *titikDb = (idx == 0) ? "ph4" : (idx == 1) ? "ph7" : "ph10";
  double ph = 0, mv = NAN; bool wasStable = false;
  if (!collectStableReading(ph, mv, wasStable, label, titikDb, "verifikasi")) return;
